...
struct HomeAllBooksScreen: View {
    ...
    private func sortBooks() {
        if !query.isEmpty {
            var newBooks: [Card] = []
            if defaultBooks.count == 0 {
                defaultBooks = books
                for book in books {
                    if book.title.lowercased().contains(query.lowercased()) {
    ...
    var body: some View {
        VStack {
            ...
            if books.count != 0 {
                CardsDoubleView(books: books)
            } else {
                Spacer()
                Image(systemName: "shippingbox.fill")
                    .font(.system(size: 100))
                    .foregroundColor(.orange)
                Text("Sorry, we don't have any books for you!")
                    .padding()
                Spacer()
            }
...
struct HomeAllBooks_Previews: PreviewProvider {
